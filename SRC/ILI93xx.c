...
{ 	 
	...
	GPIO_Set(GPIOF,PIN13,GPIO_MODE_OUT,GPIO_OTYPE_PP,GPIO_SPEED_50M,GPIO_PUPD_PU);							//PF13 推挽输出,控制背光
...
/*****************************************************************************
 * @name       :void Lcd_WriteData_16Bit2(u16 Data)
 * @date       :2018-08-09 
 * @function   :Write an 16-bit command to the LCD screen
 * @parameters :Data:Data to be written
 * @retvalue   :None
******************************************************************************/	 
void Lcd_WriteData_16Bit2(u16 Data)
{
...
/*****************************************************************************
 * @name       :void LCD_SetWindows2(u16 xStar, u16 yStar,u16 xEnd,u16 yEnd)
 * @date       :2018-08-09 
 ...
 * @retvalue   :None
******************************************************************************/ 
void LCD_SetWindows2(u16 xStar, u16 yStar,u16 xEnd,u16 yEnd)
{	
	LCD_WR_REG(lcddev.setxcmd);	
	LCD_WR_DATA(xStar>>8);
...
/*****************************************************************************
 * @name       :void LCD_ShowChar2(u16 x,u16 y,u16 fc, u16 bc, u8 num,u8 size,u8 mode)
 * @date       :2018-08-09 
 ...
 * @retvalue   :None
******************************************************************************/ 
void LCD_ShowChar2(u16 x,u16 y,u16 fc, u16 bc, u8 num,u8 size,u8 mode)
{  
    u8 temp;
    u8 pos,t;
	...
	num=num-' ';//得到偏移后的值
	LCD_SetWindows2(x,y,x+size/2-1,y+size-1);//设置单个文字显示窗口
	if(!mode) //非叠加方式
	{		
		for(pos=0;pos<size;pos++)
		{
			...
			for(t=0;t<size/2;t++)
		    {                 
		        if(temp&0x01) Lcd_WriteData_16Bit2(fc); 
				else Lcd_WriteData_16Bit2(bc); 
				temp>>=1; 
	...
	LCD_SetWindows2(0,0,lcddev.width-1,lcddev.height-1);//恢复窗口为全屏    	   	 	  
}
 ...
 * @retvalue   :None
******************************************************************************/  			 
void LCD_ShowNum2(u16 x,u16 y,u32 num,u8 len,u8 size)
{         	
	u8 t,temp;
	u8 enshow=0;						   
	for(t=0;t<len;t++)
	{
		...
		if(enshow==0&&t<(len-1))
		{
			if(temp==0)
			{
				LCD_ShowChar2(x+(size/2)*t,y,POINT_COLOR,BACK_COLOR,' ',size,0);
				continue;
			}else enshow=1; 
		 	 
		}
	 	LCD_ShowChar2(x+(size/2)*t,y,POINT_COLOR,BACK_COLOR,temp+'0',size,0); 
	}
}
...
void GUI_DrawFont16(u16 x, u16 y, u16 fc, u16 bc, u8 *s,u8 mode)
{
	...
	for (k=0;k<HZnum;k++) 
	{
	  ...
	  { 	LCD_SetWindows2(x,y,x+16-1,y+16-1);
		    for(i=0;i<16*2;i++)
		    {
				for(j=0;j<8;j++)
		    	{	
					if(!mode) //非叠加方式
					{
						if(tfont16[k].Msk[i]&(0x80>>j))	Lcd_WriteData_16Bit2(fc);
						else Lcd_WriteData_16Bit2(bc);
					}
					else
					{
...
void GUI_DrawFont24(u16 x, u16 y, u16 fc, u16 bc, u8 *s,u8 mode)
{
	...
	HZnum=sizeof(tfont24)/sizeof(typFNT_GB24);	//自动统计汉字数目
			...
			for (k=0;k<HZnum;k++) 
			{
			  ...
			  { 	LCD_SetWindows2(x,y,x+24-1,y+24-1);
				    for(i=0;i<24*3;i++)
				    {
							for(j=0;j<8;j++)
							{
								if(!mode) //非叠加方式
								{
									if(tfont24[k].Msk[i]&(0x80>>j))	Lcd_WriteData_16Bit2(fc);
									else Lcd_WriteData_16Bit2(bc);
								}
							else
							{
 ...
 * @date       :2018-08-09 
 * @function   :Display a single 32x32 Num character
 * @parameters :x:the bebinning x coordinate of the Chinese character
                y:the bebinning y coordinate of the Chinese character
								fc:the color value of Chinese character
								bc:the background color of Chinese character
 ...
 * @retvalue   :None
******************************************************************************/ 
void GUI_DrawNum32(u16 x, u16 y, u16 fc, u16 bc, u8 *s,u8 mode)
{
	u8 i,j;
	u16 k;
	u16 HZnum;
	u16 x0=x;
	HZnum=sizeof(NUM_font32)/sizeof(typFNT_NUM_GB32);	//自动统计汉字数目
	for (k=0;k<HZnum;k++) 
			{
			  if ((NUM_font32[k].Index[0]==*(s))&&(NUM_font32[k].Index[1]==*(s+1)))
			  { 	LCD_SetWindows2(x,y,x+32-1,y+32-1);
				    for(i=0;i<32*4;i++)
				    {
						for(j=0;j<8;j++)
				    	{
							if(!mode) //非叠加方式
							{
								if(NUM_font32[k].Msk[i]&(0x80>>j))	Lcd_WriteData_16Bit2(fc);
								else Lcd_WriteData_16Bit2(bc);
							}
							else
							{
								POINT_COLOR=fc;
								if(NUM_font32[k].Msk[i]&(0x80>>j))	LCD_DrawPoint(x,y);//画一个点
								x++;
								if((x-x0)==32)
								{
									x=x0;
			  ...
			  { 	LCD_SetWindows2(x,y,x+32-1,y+32-1);
				    for(i=0;i<32*4;i++)
				    {
						for(j=0;j<8;j++)
				    	{
							if(!mode) //非叠加方式
							{
								if(tfont32[k].Msk[i]&(0x80>>j))	Lcd_WriteData_16Bit2(fc);
								else Lcd_WriteData_16Bit2(bc);
							}
							else
							{
        ...
        if(!bHz)
        {
	        ...
	        {          
		        ...
		        else
				{
					...
					{  
					LCD_ShowChar2(x,y,fc,bc,*str,16,mode);
					x+=8; //字符,为全字的一半 
					}
					else
					{
					LCD_ShowChar2(x,y,fc,bc,*str,size,mode);
					x+=size/2; //字符,为全字的一半 
					}
				} 
				str++; 
...
{
	//主题
	Gui_StrCenter(0,0,lcddev.width,lcddev.height,"智能药箱        ",32,1);//居中显示
	//Gui_StrCenter(0,100,lcddev.width,lcddev.height,"药品计量设置    ",24,1);//居中显示
	Show_Str(0,100,lcddev.width,lcddev.height,"药品计量设置    ",24,1);
	
	//药片种类
	Show_Str(0,145,lcddev.width,lcddev.height,"药品一",16,1);
	Show_Str(0,195,lcddev.width,lcddev.height,"药品二",16,1);
	Show_Str(0,245,lcddev.width,lcddev.height,"药品三",16,1);
	Show_Str(0,295,lcddev.width,lcddev.height,"袋装药",16,1);
	
	//温湿度
	Show_Str(160,175,lcddev.width,lcddev.height,"温度",16,1);
	Show_Str(160,225,lcddev.width,lcddev.height,"湿度",16,1);
	
//	//初始时间打印
//	LCD_ShowNum(1*lcddev.width/4,60,00,2,16);
//	LCD_ShowNum(2*lcddev.width/4,60,00,2,16);
//	LCD_ShowNum(3*lcddev.width/4,60,00,2,16);
}
...
{
	//打印药片数量
	LCD_ShowNum(100,145,pill1,1,16);
	LCD_ShowNum(100,195,pill2,1,16);
	LCD_ShowNum(100,245,pill3,1,16);
	LCD_ShowNum(100,295,pill4,1,16);
	
	//打印温湿度
	LCD_ShowNum(200,175,0,2,16);	//显示温度	  大米存储bai温度在10℃以下，湿度在du12%以下。 		   
	
	LCD_ShowNum(200,225,0,2,16);		//显示湿度
